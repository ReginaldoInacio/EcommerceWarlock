...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let buttons = [
    ...document.querySelectorAll('.corpo__lista__conteudo__item__botoes')
  ]
  buttons.map(button => {
...
const search = e => {
  let allItemTitles = [
    ...document.querySelectorAll('.corpo__lista__conteudo__item__titulo')
  ]
  if (e.keyCode == 8) {
    searchBarText.pop()
    ...
    if (!title.innerText.toLowerCase().includes(currentSearchBarText)) {
      ...
      if (
        !title.parentElement.classList.contains(
          'corpo__produto__container__info__titulo--hidden'
        )
      ) {
        title.parentElement.className =
          'corpo__produto__container__info__titulo--hidden'
      }
    } else {
      if (
        ...
        title.parentElement.className =
          'corpo__lista__conteudo__item corpo__lista__conteudo__item--todos'
      }
    }
  })
...
const removeLocalElement = e => {
  ...
  switch (type) {
    case 'trash':
      ...
      let elementName = element.querySelector(
        '.corpo__lista__conteudo__item__titulo'
      ).innerText
...
const addElements = () => {
  ...
  let location = contentSections[contentSections.length - 1].querySelector(
    '.corpo__lista__conteudo'
  )
